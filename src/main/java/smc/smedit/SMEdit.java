 ...
 *
 * <p>Applies the saved theme, creates the local SMEdit directories, and opens the
 * main editor window ({@link smc.smedit.ui.RenderFrame}). Configuration lives in
 * the in-app Settings dialog ({@link smc.smedit.ui.SettingsDialog}).
 *
 ...
 *
 * @Author Jo Jaquinta for SMEdit Classic - version 1.0
 * @Author Robert Barefoot for SMEdit - version 1.1
 */
public class SMEdit {
    ...
    public static void main(String[] args) {
        ...
        // the editor window), so it must run on the Event Dispatch Thread.
        SwingUtilities.invokeLater(() -> {
            // Apply the saved look-and-feel before any Swing component is created
            // (the folder chooser below is the first one). Configuration now lives
            // in the in-app Settings dialog rather than a startup options screen.
            setupLookAndFeel();
            ...
            // preLoad() resolves the StarMade base dir into StarMadeLogic, then
            // opens the editor (this is exactly what the old "Start SMEdit" did).
            RenderFrame.main(args);
        });
    }
